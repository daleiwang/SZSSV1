...
+ (void)initialize {
    if (self == [SSObject self]) {
    
    }
}

- (void)mappingDidComplete {
    NSDictionary *classMap = @{@"__NSCFNumber" : @"NSNumber",
    ...
    if(self.unmappedEntries.count == 0)
        return;
        ...
        if (propName) {
            ...
            if (value && (id)value != [NSNull null]) {
                ...
                if ([value isKindOfClass:[NSString class]] || [value isKindOfClass:[NSNumber class]]) {
                    ...
                    [toMapArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                         if ([obj isKindOfClass:[NSString class]] ||
                             [obj isKindOfClass:[NSNumber class]]) {
                             [array addObject:obj];
                         } else if ([obj isKindOfClass:[SSObject class]]) {
                             [array addObject:[(SSObject *)obj objectAsDictionary]];
                         } else {
                             NSLog(@"Property %@ is of type %@ which is unknown",propertyName, NSStringFromClass([value class]));
                         }
                     }];
                    ...
                    [toMapDict enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
                        if ([value isKindOfClass:[NSString class]] ||
                            [value isKindOfClass:[NSNumber class]]) {
                            [toMapDict setValue:obj forKey:key];
                        } else if ([obj isKindOfClass:[SSObject class]]) {
                            [toMapDict setValue:[(SSObject *)value objectAsDictionary] forKey:key];
                        } else {
                            NSLog(@"Property %@ is of type %@ which is unknown",propertyName, NSStringFromClass([value class]));
                        }
                    }];
                    ...
                    if ([value respondsToSelector:@selector(objectAsDictionary)]) {
                        [dict setValue:[(SSObject *)value objectAsDictionary] forKey:propertyName];
                    } else {
...
- (id)transformedJSONObjectForJSONObject:(id)jsonObject {
    ...
    if ([jsonObject isKindOfClass:[NSDictionary class]]) {
        ...
        [jsonObject enumerateKeysAndObjectsUsingBlock:^(id key, id obj, BOOL *stop) {
            ...
            NSString *camelCaseKey = [key stringByReplacingCharactersInRange:NSMakeRange(0, 1) withString:[[key substringToIndex:1] lowercaseString]];
...
- (void)setValue:(id)value forKey:(NSString *)key {
    ...
    if([classesForMapping.allKeys containsObject:key]) {
        ...
        if(classToUse) {
            value = [SSObject map:value usingClass:classToUse];
        }
    }
